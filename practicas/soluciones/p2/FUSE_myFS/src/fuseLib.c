...
static int my_unlink(const char *path) {
	...
	/* Ponemos todo el bloque a ceros para borrar de disco la información de los bloques del fichero*/
	memset(bloque, 0, sizeof(char)*BLOCK_SIZE_BYTES);
	fprintf(stderr, "--->>> my_unlink: path %s\n", path);

	/* Buscamos nodo-i en fichero */
	if((idxNodoI = findFileByName(&myFileSystem, (char*) path+1)) == -1)
		return -ENOENT;

	/* En BitMap, liberamos todos los bloques del fichero */
	for(i = 0; i < myFileSystem.nodes[idxNodoI]->numBlocks; i++) {
		...
		if( (lseek(myFileSystem.fdVirtualDisk, bloqueActual * BLOCK_SIZE_BYTES, SEEK_SET) ==(off_t) - 1) ||
			...
			perror("error lseek/write en un-link");
			return -EIO;
		}
	}
	...
	/* en Directory, actualizamos el directorio */
	myFileSystem.directory.files[idxNodoI].freeFile = 1;
	...
	/* En nodos-i en disco, por un lado, liberaramos el nodo-i del fichero a borrar y guardamos en disco */
	myFileSystem.nodes[idxNodoI]->freeNode = 1;
	updateNode(&myFileSystem, idxNodoI, myFileSystem.nodes[idxNodoI]);

	/* En nodos-i en memoria, por otro lado, liberamos el espacio ocupado por el nodo-i anterior y apuntamos a NULL */
	free(myFileSystem.nodes[idxNodoI]);
	myFileSystem.nodes[idxNodoI] = NULL;

	/* Actualizamos nodos libres */
	myFileSystem.numFreeNodes++;

	/* En SuperBloque, actualizamos bloques libres */
...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	nodoI->modificationTime = time(NULL);

	//Guardamos en disco lo de las estructuras modificadas
	updateSuperBlock(&myFileSystem);
	updateBitmap(&myFileSystem);
	updateNode(&myFileSystem, fi->fh, nodoI);
	sync();
	return size;
}
